...
If the user names a specific artifact (a file, a document, something produced earlier in the conversation), sanitize that.

If no target is specified, derive candidates from the git diff:

1. Run `git diff --name-only` and `git diff --name-only --cached` for uncommitted changes. If both are empty, diff the current branch against the default branch (e.g. `git diff --name-only develop...HEAD` or `main...HEAD`).
2. From the changed files, keep user-facing artifact files: documentation, guides, README files, and similar deliverables (typically `.md`, `.mdx`, `.txt`, `.html`). Exclude source code, configuration, lockfiles, and test files unless the user asks for them.
3. If this yields no candidates, ask the user what to sanitize instead of guessing.
4. If it yields multiple candidates, list them briefly and confirm before editing, unless the user clearly asked for all of them.

The diff is used only to identify targets. When revising, always read and treat each target file as a whole — checking tone, terminology, and voice across the entire file — not just the changed lines. Partial fixes leave the patchwork feel this skill exists to remove.
